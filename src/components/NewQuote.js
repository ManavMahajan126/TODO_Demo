...
export default function NewQuote(props) {
    ...
    const onSave = () => {
        ...
        if (edit) {
            ...
            let id = generateID();
            quote_ = {"id": id, "title": title, "text": text};
        }
    ...
    //4 - RENDER
    let disabled = (title.length > 0 && text.length > 0) ? false : true;
    return (
            ...
            <SafeAreaView style={styles.flex}>
                ...
                <View style={styles.buttonContainer}>
                    <View style={{flex: 1, justifyContent: "center"}}>
                        <Text style={[styles.count, (MAX_LENGTH - text.length <= 10) && {color: "red"}]}> Length: {MAX_LENGTH - text.length}</Text>
                    </View>
